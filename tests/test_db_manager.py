...
def test_create_employers_table(db_manager, params_with_db_name):
    ...
    with psycopg2.connect(**params_with_db_name) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'employers'"
            )
...
def test_create_vacancies_table(db_manager, params_with_db_name):
    ...
    with psycopg2.connect(**params_with_db_name) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'vacancies'"
            )
            columns = cur.fetchall()
...
def test_get_companies_and_vacancies_count(
    database_with_tables, db_manager, params_with_db_name
):
    with psycopg2.connect(**params_with_db_name) as conn:
        with conn.cursor() as cur:
            assert db_manager.get_companies_and_vacancies_count(cur) == [
                ("employer_1", 2)
            ]
...
def test_get_all_vacancies(database_with_tables, db_manager, params_with_db_name):
    with psycopg2.connect(**params_with_db_name) as conn:
        with conn.cursor() as cur:
            assert db_manager.get_all_vacancies(cur)[0] == (
                "employer_1",
                "vacancy_1",
                30000,
                50000,
            ...
            assert db_manager.get_all_vacancies(cur)[1] == (
                "employer_1",
                "vacancy_2",
                50000,
                70000,
